...
class course_format_data_common_trait {
    ...
    private function __construct() {
        $this->plugin_config = "format_vums";
        $this->modstats = \format_vums\ModStats::getinstance();
    }
    /**
    ...
    public function display_file($context, $itemid) {
        ...
        $files = get_file_storage()->get_area_files(
            $context->id, 'format_vums', 'vumscourseimage_filearea',
     ...
     * @param  object          $renderer       vums renderer object.
     ...
     * @param  format_vums     $courseformat   Course format object
    ...
    public function get_all_section_data($renderer, $editing, $rformat, $settings, $course, $courseformat, $courserenderer) {
        ...
        for ($sectionindex = $startfrom; $sectionindex <= $end; $sectionindex++) {
            ...
            if (!$editing) {
                ...
                $label = html_writer::span(get_string('addnewsection', 'format_vums'), 'wdmaddsection d-none d-lg-block');
            ...
            $vumsenablecardbackgroundimg = $settings['vumsenablecardbackgroundimg'];
            $vumsdefaultsectiontheme = 'dark';
            ...
            if ($rformat == VUMS_CARD_FORMAT) {
                ...
                $vumsdefaultsectionmode = "";
                if ( $vumsenablecardbackgroundimg == 1 && $this->get_section_first_image( $section, $section->summary )) {
                    $vumsnewthemecolor = 'dark';
                    $vumsdefaultsectionmode = true;
                    $vumsnewfontcolor = '#eaeaea';
                    ...
                    $vumsdefaultsectionoverlay = "180deg, rgba(255, 255, 255, 0) 0%, #000000 110%";
                    $data->vumsdefaultsectionmode = $vumsdefaultsectionmode;
                    $data->vumsdefaultsectionoverlay = $vumsdefaultsectionoverlay;
                    $data->vumsnewfontcolor = $vumsnewfontcolor;
                    $data->vumsnewthemecolor = $vumsnewthemecolor;
                }
                ...
                $sections[] = $data;
            } else if ($rformat == VUMS_LIST_FORMAT) {
                ...
                //);
                $data->addnewactivity = $courserenderer->course_section_add_cm_control(
                    $course,
                    $section->section,
                    0
                );
    ...
    public function get_section_module_info($section, $course, $mods, $singlepageurl) {
        ...
        if ($total > 0) {
            ...
            if ($pinfo->percentage == 0) {
                ...
                get_string('activitystart', 'format_vums') . '</a>';
            } else if ( $pinfo->percentage > 0 && $pinfo->percentage < 50 ) {
                if ($total == 1) {
                    $status = get_string('activitycompleted', 'format_vums');
                } else {
                    $status = get_string('activitiescompleted', 'format_vums');
                }
                $pinfo->progress = $total . $status;
                $pinfo->progress = '<a href=' . $singlepageurl . '>' . $complete . ' '
                                    . get_string('outof', 'format_vums') . ' '
                                    . $total . ' ' . $status . '</a>';
            } else if ( $pinfo->percentage >= 50 && $pinfo->percentage < 100 ) {
                $total = $total - $complete;
                if ($total == 1) {
                    $status = get_string('activityremaining', 'format_vums');
                } else {
                    $status = get_string('activitiesremaining', 'format_vums');
                }
                $pinfo->progress = '<a href=' . $singlepageurl . '>' . $total . ' ' . $status . '</a>';
            } else if ( $pinfo->percentage == 100 ) {
                $pinfo->progress = get_string('allactivitiescompleted', 'format_vums');
            }
            if ($pinfo->percentage == 0) {
    ...
    public static function get_dummy_image_for_id($id) {
        global $OUTPUT;
        return $OUTPUT->image_url('banner', 'format_vums');
    ...
    public function get_activity_to_resume($course) {
        ...
        // Fetch last viewed from vums_course_visits table.
        $lastviewed = $DB->get_record('vums_course_visits',
            array(
                'course' => $course->id,
                'userid' => $USER->id
            )
        ...
        // Fetch last viewed from log if not record in vums_course_visits.
    ...
    public function get_section_first_image($section, $summaryhtml) {
        ...
        if ($section->section >= 1 && $section->section <= 6) {
            ...
            $imgarray['img'] = $OUTPUT->image_url('section'.$section->section, 'format_vums');
        ...
        // @codingStandardsIgnoreStart
        $imgarray['img'] = $OUTPUT->image_url('pattern', 'format_vums');
        // @codingStandardsIgnoreEnd
        $imgarray['pattern'] = 1;
        return $imgarray;
    }
    ...
    public function add_generalsection_data(&$export, $renderer, $editing, $course, $courseformat, $courserenderer) {
        ...
        if ($generalsection) {
            ...
            $export->generalsection['starttime'] = userdate($course->startdate,get_string('strftimedatemonthabbr','langconfig'));
            ...
            $export->generalsection['coursemainimage'] = '/theme/pafco/pix/course-default.jpg';
            ...
            get_vums_extra_header_context($export, $course, progress::get_course_progress_percentage($course), $imgurl);
            ...
            $export->introductionsection['course_introduction'] = is_array($settings['vums_introduction_editor'])
                ? $settings['vums_introduction_editor']['text']
                : $settings['vums_introduction_editor'];
            $export->introductionsection['vums_teaching_group'] = is_array($settings['vums_teaching_group_editor'])
                ? $settings['vums_teaching_group_editor']['text']
                : $settings['vums_teaching_group_editor'];
            $export->introductionsection['vums_assessment'] = is_array($settings['vums_assessment_editor'])
                ? $settings['vums_assessment_editor']['text']
                : $settings['vums_assessment_editor'];

            $export->blocksection = $OUTPUT->blocks('course-vums');
            $export->addblock = $OUTPUT->addblockbutton('course-vums');
        }
    }
