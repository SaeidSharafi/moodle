...
require_once($CFG->dirroot.'/course/format/vums/lib.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_vums_card_all_sections_summary implements renderable, templatable {
    ...
    public function __construct($course, $renderer) {
        ...
        $this->courseformatdatacommontrait = \format_vums\course_format_data_common_trait::getinstance();
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $export->courseformat = get_config('format_vums', 'defaultcourseformat');
        $export->theme = $PAGE->theme->name;

        if ($editing) {
            $PAGE->requires->js_call_amd('format_vums/card_editing_observer', 'init');
        }
        if ($rformat == VUMS_CARD_FORMAT) {
            $PAGE->requires->js_call_amd('format_vums/format_card', 'init');
     ...
     * @param format_vums $renderer Format renderer object
