...
namespace format_vums\observers;
...
class course_module {
    ...
    public static function viewed(\core\event\course_module_viewed $event) {
        ...
        // Check if format set to vums.
        if ($course->format != 'vums') {
            return;
        }

        // Update cm id if course and user record is present.
        if ($visit = $DB->get_record('vums_course_visits', array('course' => $courseid, 'userid' => $userid))) {
            $visit->cm = $cmid;
            $visit->timevisited = time();
            $DB->update_record('vums_course_visits', $visit);
            return;
        }
        ...
        $visit->cm = $cmid;
        $visit->timevisited = time();
        $DB->insert_record('vums_course_visits', $visit);
    }
}
