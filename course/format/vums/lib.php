...
require_once($CFG->dirroot.'/course/format/lib.php'); // For format_vums.

define('VUMS_CARD_FORMAT', 0);
define('VUMS_LIST_FORMAT', 1);

/**
 * Cards Format - A topics based format that uses card layout to display the activities/section/topics.
 */
class format_vums extends core_courseformat\base
{
    ...
    protected function __construct($format, $courseid)
    {
        ...
        $this->availablelayouts = array(
            'VUMS_CARD_FORMAT' => array(
                'format'      => VUMS_CARD_FORMAT,
                'optionlabel' => 'vumscourseformat_card',
                'supports'    => COURSE_DISPLAY_MULTIPAGE,
            ),
            'VUMS_LIST_FORMAT' => array(
                'format'      => VUMS_LIST_FORMAT,
                'optionlabel' => 'vumscourseformat_list',
    ...
    public function get_settings()
    {
        if (empty($this->settings) == true) {
            ...
            $this->settings['vumscourseimage_filemanager'] = $this->get_vumscourseimage_filemanager();
        }
     ...
     * If the section number is not 0, the base implementation of format_vums::get_default_section_name which
     * uses the string with the key = 'sectionname' from the course format's lang file + the section number will be
     * used.
     *
    ...
    public function get_default_section_name($section)
    {
        if ($section->section == 0) {
            ...
            return get_string('section0name', 'format_vums');
        }
        if ($section->section == 1) {
            // Return the general section.
            return get_string('section1name', 'format_vums');
        }
        if ($section->section == 2) {
            // Return the general section.
            return get_string('section2name', 'format_vums');
        }
        if ($section->section == 3) {
            // Return the general section.
            return get_string('section3name', 'format_vums');
        }
        if ($section->section == 4) {
            // Return the general section.
            return get_string('section4name', 'format_vums');
        }
        if ($section->section == 5) {
            // Return the general section.
            return get_string('section5name', 'format_vums');
        }
        if ($section->section == 6) {
            // Return the general section.
            return get_string('section6name', 'format_vums');
        }
        if ($section->section == 7) {
            // Return the general section.
            return get_string('section7name', 'format_vums');
        }
        // Use format_vums::get_default_section_name implementation which
        // will display the section name in "Topic n" format.
        return get_string('sectionname', 'format_vums').' '.$section->section;
    ...
    public function hide_general_section_when_empty($course, $modinfo = false)
    {
        ...
        $hidegeneralsectionwhenempty = gettype($settings['hidegeneralsectionwhenempty']) == 'integer'
            ...
            :
            (get_config('format_vums', 'hidegeneralsectionwhenempty') || false);

        if ($sectioninfo->summary || !empty($modinfo->sections[0]) || $PAGE->user_is_editing()
    ...
    public function course_format_options($foreditform = false)
    {
        ...
        if ($courseformatoptions === false) {
            ...
            $courseformatoptions = array(
                'vums_introduction_editor'     => array(
                    'default' => "",
                    'type'    => PARAM_RAW
                ),
                'vums_teaching_group_editor'   => array(
                    'default' => "",
                    'type'    => PARAM_RAW
                ),
                'vums_assessment_editor'       => array(
                    'default' => "",
                    'type'    => PARAM_RAW
                ),
                ...
                'vumscourseformat'             => array(
                    'default' => $defaultlayout,
                    'type'    => PARAM_INT
                ),
                'hidegeneralsectionwhenempty'  => array(
                    'default' => get_config('format_vums', 'hidegeneralsectionwhenempty') || false,
                    'type'    => PARAM_INT
                ),
                'coursedisplay'                => array(
                    'default' => 0,
                ...
                'vumscourseimage_filemanager'  => array(
                    'default' => false,
                    'type'    => PARAM_INT
                ),
                'sectiontitlesummarymaxlength' => array(
                    'default' => get_config('format_vums', 'defaultsectionsummarymaxlength'),
                    'type'    => PARAM_INT
                ),
        ...
        if ($foreditform && !isset($courseformatoptions['coursedisplay']['label'])) {
            ...
            $courseformatoptionsedit = array(
                ...
                'vumscourseformat'             => array(
                    'label'              => new lang_string('vumscourseformat', 'format_vums'),
                    'element_type'       => 'select',
                    'element_attributes' => array(
                        array(
                    ...
                    'help'               => 'vumscourseformat',
                    'help_component'     => 'format_vums',
                ),
                'hidegeneralsectionwhenempty'  => array(
                    'label'              => new lang_string('hidegeneralsectionwhenempty', 'format_vums'),
                    'element_type'       => 'select',
                    'element_attributes' => array(
                        array(
                ...
                'vumscourseimage_filemanager'  => array(
                    'label'              => new lang_string('vumscourseimage_filemanager', 'format_vums'),
                    ...
                    'help'               => 'vumscourseimage_filemanager',
                    'help_component'     => 'format_vums',
                ),
                'sectiontitlesummarymaxlength' => array(
    ...
    /**
     * DB value setter for vumscourseimage_filemanager option
     *
     * @param  boolean  $itemid  Image itemid
     */
    public function set_vumscourseimage_filemanager($itemid = false)
    {
        global $DB;
        $courseimage = $DB->get_record('course_format_options', array(
            ...
            'format'    => 'vums',
            'sectionid' => 0,
            'name'      => 'vumscourseimage_filemanager'
        ));
     ...
     * @return int Item id
     */
    public function get_vumscourseimage_filemanager()
    {
        global $DB;
    ...
    public function get_vumscourseimage_filemanagers()
    {
        global $DB;
        $itemid = $DB->get_field('course_format_options', 'value', array(
            'courseid'  => $this->courseid,
    ...
    public function create_edit_form_elements(&$mform, $forsection = false)
    {
        ...
        $data = new stdClass;
        $fileitemid = $this->get_vumscourseimage_filemanager();
        ...
        if (!$mform->isSubmitted()) {
            $data->vums_introduction = $settings['vums_introduction_editor']['text'] ?? null;
            $data->vums_teaching_group = $settings['vums_teaching_group_editor']['text'] ?? null;
            $data->vums_assessment = $settings['vums_assessment_editor']['text'] ?? null;
            ...
            $data = file_prepare_standard_editor(
                ...
                $coursecontext,
                'format_vums',
                'vums_assessment_filearea',
                0
            );

            $mform->setDefault('vums_introduction_editor', $data->vums_introduction_editor);
            $mform->setDefault('vums_teaching_group_editor', $data->vums_teaching_group_editor);
            $mform->setDefault('vums_assessment_editor', $data->vums_assessment_editor);
        }
        ...
        //    'context' => $coursecontext,
        //];
        //$mform->setDefault('vums_introduction_editor', $vums_introduction_editor);

        //$vums_teaching_group = $settings['vums_teaching_group_editor'];
        ...
        foreach ($elements as $key => $element) {
            ...
            if (
                $element->getName() === 'vums_introduction_editor'
                || $element->getName() === 'vums_teaching_group_editor'
                || $element->getName() === 'vums_assessment_editor'
            ) {
    ...
    public function inplace_editable_render_section_name(
        ...
        if (empty($edithint)) {
            $edithint = new lang_string('editsectionname', 'format_vums');
        }
        if (empty($editlabel)) {
            $title = get_section_name($section->course, $section);
            $editlabel = new lang_string('newsectionname', 'format_vums', $title);
        }
        return parent::inplace_editable_render_section_name($section, false, false, $edithint, $editlabel);
    }
    ...
    {
        global $USER;
        if (!isset($data->vumscourseimage_filemanager)) {
        ...
        if (!empty($data)) {
            ...
            if (!empty($data->vumscourseimage_filemanager)) {
            ...
            if (isset($data->vums_introduction_editor)) {

                $data = file_postupdate_standard_editor(
                    $data,
                    'vums_introduction',
                    ['subdirs' => false, 'maxfiles' => -1, 'context' => $contextid],
                    $contextid,
                    'format_vums',
                    'vums_introduction_filearea',
                    $data->vums_introduction_editor['itemid']
                );

                $data->vums_introduction = file_rewrite_pluginfile_urls(
                    $data->vums_introduction,
                    'pluginfile.php',
                    $contextid->id,
                    'format_vums',
                    'vums_introduction_filearea',
                ...
                $data->vums_introduction_editor['text'] = $data->vums_introduction;
            }
            if (isset($data->vums_teaching_group_editor)) {
                $data = file_postupdate_standard_editor(
                    $data,
                    'vums_teaching_group',
                    ...
                    'format_vums',
                    'vums_teaching_group_filearea',
                    $data->vums_teaching_group_editor['itemid']
                );
                  $data->vums_teaching_group = file_rewrite_pluginfile_urls(
                    $data->vums_teaching_group,
                    'pluginfile.php',
                    $contextid->id,
                    'format_vums',
                    'vums_teaching_group_filearea',
                ...
                $data->vums_teaching_group_editor['text'] = $data->vums_teaching_group;
            }
            if (isset($data->vums_assessment_editor)) {
                $data = file_postupdate_standard_editor(
                    $data,
                    'vums_assessment',
                    ['subdirs' => false, 'maxfiles' => -1],
                    ...
                    'format_vums',
                    'vums_assessment_filearea',
                    $data->vums_assessment_editor['itemid']
                );
                  $data->vums_assessment = file_rewrite_pluginfile_urls(
                    $data->vums_assessment,
                    'pluginfile.php',
                    $contextid->id,
                    'format_vums',
                    'vums_assessment_filearea',
                ...
                $data->vums_assessment_editor['text'] = $data->vums_assessment;
            }
 ...
 * @return \core\output\inplace_editable
 */
function format_vums_inplace_editable($itemtype, $itemid, $newvalue)
{
    global $DB, $CFG;
    ...
    if ($itemtype === 'sectionname' || $itemtype === 'sectionnamenl') {
        $section = $DB->get_record_sql(
            ...
            array($itemid, 'vums'), MUST_EXIST);
        return course_get_format($section->course)->inplace_editable_update_section_name($section, $itemtype,
            $newvalue);
    }
}

/**
 * Serves file from vumscourseimage_filearea
 *
 ...
 */
function format_vums_pluginfile(
    $course,
    $cm,
    $context,
    $filearea,
    ...
    require_login();
    if (
        $filearea != 'vumscourseimage_filearea'
        && $filearea != 'vums_introduction_filearea'
        && $filearea != 'vums_assessment_filearea'
        && $filearea != 'vums_teaching_group_filearea'
    ) {
        return false;
    }
    ...
    $file = $fs->get_file($context->id, 'format_vums', $filearea, $itemid, $filepath, $filename);
...
function format_vums_check_plugin_available($component)
{
 ...
 */
function get_enrolled_teachers_context_formated($courseid = null, $frontlineteacher = false)
{
    global $OUTPUT;
    ...
    if ($teachers) {
        ...
        foreach ($teachers as $key => $teacher) {
            if(!user_has_role_assignment($teacher->id,3,$coursecontext->id)){
                continue;
            }
            $hasTeachers = true;
        ...
        $context['hasteachers'] = $hasTeachers;
    }
    return $context;
}

function get_enrolled_students_stats($courseid = null)
{
    global $DB;
    $coursecontext = \context_course::instance($courseid);
 ...
 */
function formated_get_course_image($corecourselistelement, $islist = false)
{
    global $CFG, $OUTPUT;
...
function get_vums_extra_header_context(&$export, $course, $percentage, $imgurl)
{
    ...
    $rnrshortdesign = '';
    if (format_vums_check_plugin_available("block_edwiserratingreview")) {
    ...
    $export->generalsection['teachers'] = get_enrolled_teachers_context_formated($course->id, true);
    $export->generalsection['studentscount'] = get_enrolled_students_stats($course->id);
    ...
    if (gettype($imgurl) != "object") {
        $imgurl = formated_get_course_image($course);
    }
    $export->generalsection['headercourseimage'] = $imgurl;
    $export->generalsection['vumsheaderimagebgposition'] = $coursesettings['edw_format_hd_bgpos'];
    $export->generalsection['vumsheaderimagebgsize'] = $coursesettings['edw_format_hd_bgsize'];
    $export->generalsection['courseheaderdesign'] = true;
    $export->turneditingonswitch = $OUTPUT->page_heading_button();
    if ($CFG->theme == 'vums') {
        $export->generalsection['courseheaderdesign'] = get_config('theme_vums', 'courseheaderdesign') == 0 ? false
            : true;
