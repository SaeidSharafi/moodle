...
function xmldb_format_vums_upgrade($oldversion)
{
    ...
    if ($oldversion < 2020061800) {
        // Define table format_vums to be created.
        $table = new xmldb_table('format_vums');

        // Adding fields to table format_vums.
        ...
        // Adding keys to table format_vums.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);
        $table->add_key('courseid', XMLDB_KEY_FOREIGN, ['courseid'], 'course', ['id']);

        // Adding indexes to table format_vums.
        $table->add_index('formatoption', XMLDB_INDEX_UNIQUE, ['courseid', 'sectionid', 'activityid', 'layouttype']);

        // Conditionally launch create table for format_vums.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // vums savepoint reached.
        upgrade_plugin_savepoint(true, 2020061800, 'format', 'vums');
    }

    if ($oldversion < 2021070800) {
        // Define table vums_course_module_visits to be created.
        $table = new xmldb_table('vums_course_visits');

        // Adding fields to table vums_course_module_visits.
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        ...
        // Adding keys to table vums_course_module_visits.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);

        // Adding indexes to table vums_course_module_visits.
        $table->add_index('vumsvisits', XMLDB_INDEX_UNIQUE, ['course', 'userid']);

        // Conditionally launch create table for vums_course_module_visits.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // vums savepoint reached.
        upgrade_plugin_savepoint(true, 2021070800, 'format', 'vums');
    }
