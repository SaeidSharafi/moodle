...
require_once($CFG->dirroot. '/theme/pafco/ccn/rating_handler/ccn_rating_handler.php');
  ...
  public function ccnGetUserDetails($userId) {
    ...
    if ($DB->record_exists('user', array('id' => $userId))) {
      ...
      if($moreUserData->deleted !== '1'){
        ...
        foreach($teachingCourses as $key => $course) {
          ...
          if ($DB->record_exists('course', array('id' => $courseID))) {
            ...
            if($PAGE->theme->settings->course_ratings == 2){
              $ratingBlock = block_instance('pafco_course_rating');
        ...
        $ccnRender->profileCount = $ccnProfileCount . ' '. get_string('profile_views', 'theme_pafco');
