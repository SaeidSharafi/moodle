...
class ccnCourseHandler {
    ...
    if ($DB->record_exists('course', array('id' => $courseId))) {
      ...
      foreach ($enrolInstances as $key => $instance) {
        if(!empty($instance->cost)){
          ...
          $ccnCurrency = !empty($instance->currency) ? $instance->currency : get_string('currency', 'theme_pafco');
          /* @ccnBreak */
      ...
      foreach ($ccnEnrolmentCosts as $key => $cost) {
        ...
        $theLocale = 'en';
        $theCurrency = !empty($cost->currency) ? $cost->currency : get_string('currency', 'theme_pafco');
        ...
        if (class_exists('NumberFormatter')) {
          ...
          $ccnEnrolmentCosts[$key]->currencySymbol = get_string('currency_symbol', 'theme_pafco');
        }
      ...
      if (!empty($ccnEnrolmentCosts) && !empty($ccnCoursePrice)) {
        ...
        $ccnCoursePriceDisplay = get_string('course_free', 'theme_pafco');
        $ccnCourseHasCost = 1;
      } else {
        $ccnEnrolmentCosts = '';
        $ccnCoursePrice = '';
      ...
      $contentimages = $contentfiles = $CFG->wwwroot . '/theme/pafco/images/ccnBg.png';
      foreach ($courseElement->get_course_overviewfiles() as $file) {
          ...
          if ($isimage) {
              ...
              $contentfiles = $CFG->wwwroot . '/theme/pafco/images/ccnBg.png';
          }
      }

      // Incase user hasn't run DB installer yet; using old version of Pafco/Demo Installation & hasn't run DB upgrade @important!
      try {
        $ccnGetRatingInstance = block_instance('pafco_course_rating');
        $ccnProcessRatingRenderFunction = 0;
        $ccnProcessRatingCountTotalFunction = 0;
        $ccnProcessRatingCountFunction =0;
      } catch (Exception $e) {
      ...
      if($PAGE->theme->settings->coursecat_enrolments != 1){
        ...
        $ccnRender->enrolmentIcon1 = '<li class="list-inline-item"><i class="flaticon-profile"></i></li><li class="list-inline-item">'.$ccnCourse->enrolments. ' '.get_string('students_enrolled', 'theme_pafco').'</li>';
      }
      ...
      if($PAGE->theme->settings->coursecat_announcements != 1){
        ...
        $ccnRender->announcementsIcon1   =     '<li class="list-inline-item"><i class="flaticon-comment"></i></li><li class="list-inline-item">'.$ccnCourse->numberOfSections.' '.get_string('topics', 'theme_pafco').'</li>';
      }
      $ccnRender->updatedDate           =     '';
      if($PAGE->theme->settings->coursecat_modified != 1){
        $ccnRender->updatedDate         =     '<p>'.get_string('updated', 'theme_pafco').' '.userdate($courseUpdated, get_string('strftimedatefullshort', 'langconfig')).'</p>';
      }
  ...
  public function ccnBuildCourseFilterType($ccnType) {
    ...
    if(
      ...
      if(!empty(get_string('ccn_cf_show_all', 'theme_pafco'))){
        $ccnStringShowAll = get_string('ccn_cf_show_all', 'theme_pafco');
      }
  ...
  public function ccnBuildCourseFilterPrice() {
    ...
    $ccnStringPaid = 'Paid';
    $ccnStringFree = 'Free';
    if(!empty(get_string('ccn_cf_paid', 'theme_pafco'))){
      $ccnStringPaid = get_string('ccn_cf_paid', 'theme_pafco');
    }
    if(!empty(get_string('ccn_cf_free', 'theme_pafco'))){
      $ccnStringFree = get_string('ccn_cf_free', 'theme_pafco');
    }
  ...
  public function ccnBuildCourseFilterRating() {
    ...
    if(!empty(get_string('ccn_cf_1_stars', 'theme_pafco'))){
      $ccnString1 = get_string('ccn_cf_1_stars', 'theme_pafco');
    }
    if(!empty(get_string('ccn_cf_2_stars', 'theme_pafco'))){
      $ccnString2 = get_string('ccn_cf_2_stars', 'theme_pafco');
    }
    if(!empty(get_string('ccn_cf_3_stars', 'theme_pafco'))){
      $ccnString3 = get_string('ccn_cf_3_stars', 'theme_pafco');
    }
    if(!empty(get_string('ccn_cf_4_stars', 'theme_pafco'))){
      $ccnString4 = get_string('ccn_cf_4_stars', 'theme_pafco');
    }
    if(!empty(get_string('ccn_cf_5_stars', 'theme_pafco'))){
      $ccnString5 = get_string('ccn_cf_5_stars', 'theme_pafco');
    }
  ...
  public function ccnBuildFilterCoursesUrl($ccnParam, $ccnValue){
    ...
    if (
      ...
      foreach ($params as $key => $term){
        ...
        if(!isset($_GET['pafco_filter'])){
          $urlArray['pafco_filter'] = '';
        }
      }
  ...
  public function ccnGetCourseCategoryFilterCount($coursecat) {
    if($coursecat){
        ...
        if(isset($_GET['pafco_filter'])){
          $ccnCatCourseCount = get_string('ccn_cf_all_matching', 'theme_pafco');
        }
  ...
  public function ccnGetCategoryDetails($categoryId){
    ...
    if ($DB->record_exists('course_categories', array('id' => $categoryId))) {
      ...
      if ($src && $description){
        ...
        //   $countNoOfCourses = '<p>'.get_string('number_of_courses', 'theme_pafco', count($categoryCourses)).'</p>';
        // } else {
        //   $countNoOfCourses = '';
        // }
