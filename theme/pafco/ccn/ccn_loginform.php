...
include_once($CFG->dirroot.'/theme/pafco/ccn/page_handler/ccn_page_handler.php');
...
if (!isloggedin() or isguestuser()) {   // Show the block
    ...
    $_ccnlogin .= '<input type="text" name="username" placeholder="'.get_string('username', 'theme_pafco')
        .'" id="login_username" ';
    $_ccnlogin .= ' class="form-control" value="'.s($username).'" autocomplete="username"/></div>';

    $_ccnlogin .= '<div class="form-group">';

    $_ccnlogin .= '<input type="password" name="password" id="login_password" placeholder="'.get_string('password',
            'theme_pafco').'" ';
    $_ccnlogin .= ' class="form-control" value="" autocomplete="current-password"/>';
    $_ccnlogin .= '</div>';
    $otp_enabled = false;
    $authsequence = get_enabled_auth_plugins(true);
    if (in_array('otp', $authsequence)) {
        $otp_enabled = true;
    }
    ...
    if ($otp_enabled && $this->page->url->get_path() !== '/login/index.php') {
        $_ccnlogin .= '<div class="moreproviderlink">';
        $_ccnlogin .= '<a class="btn btn-primary btn-block mt-3" ';
        $_ccnlogin .= 'href="'.$CFG->wwwroot.'/auth/otp/login.php" title="'.get_string('otpbutton', 'auth_otp').'">';
        $_ccnlogin .= get_string('otpbutton', 'auth_otp').'</a>';
        $_ccnlogin .= '</div><br>';
    }
    $_ccnlogin .= "</form>\n";
