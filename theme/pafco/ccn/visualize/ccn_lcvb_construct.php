...
if($b == 0){
    ...
    $allowedPafcoBlocks = [];
    foreach($tss as $k=>$v) {
      if (strpos($k, 'pafco') !== false) {
        $tmp = new stdClass();
        $tmp->id = $v->id;
        $tmp->name = $v->name;
        $tmp->visible = $v->visible;
        $tmp->title = $v->title;
        $tmp->image = 'http://raphaelv3.sg-host.com/theme/pafco/ccn/visualize/ccn_block/jpeg/thumb' . $v->name . '.jpeg';
        $allowedPafcoBlocks[$k] = $tmp;
      }
    }


echo('<div id="json_add" style="display:none;">'.json_encode($allowedPafcoBlocks)).'</div>';




    $PAGE->requires->css('/theme/pafco/ccn/visualize/src/main.css');
    $PAGE->requires->js('/theme/pafco/ccn/visualize/src/main.js', false);
