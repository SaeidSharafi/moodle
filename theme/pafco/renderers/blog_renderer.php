...
require_once($CFG->dirroot . "/theme/pafco/ccn/blog_handler/ccn_blog_handler.php");

class theme_pafco_core_blog_renderer extends core_blog_renderer {
  ...
  public function render_blog_entry(blog_entry $entry) {
     ...
     /* $attachmentsoutputs = array();
      ...
      // CCN Attachments
      $image = $CFG->wwwroot .'/theme/pafco/images/ccnBgMd.png';
      if ($entry->renderable->attachments) {
        foreach($entry->renderable->attachments as $attachment) {
     ...
     // $o .= $this->output->tag_list(core_tag_tag::get_item_tags('core', 'post', $entry->id));
      ...
      $pafco_share_fb = 'https://www.facebook.com/sharer/sharer.php?u='. $entryurl;
      $pafco_share_tw = 'https://twitter.com/intent/tweet?url='. $entryurl;
      $pafco_share_li = 'https://www.linkedin.com/shareArticle?mini=true&url='. $entryurl;
      $pafco_share_pi = 'http://pinterest.com/pin/create/button/?url='. $entryurl;
      $pafco_share_vk = 'http://vk.com/share.php?url='. $entryurl;
      $pafco_share_em = 'mailto:?&body='. $entryurl;
      $o = '';
          ...
          <div class="mbp_thumb_post">
              ...
              <h3>'. $entry->subject .'</h3>';
                ...
                if (!empty($entry->renderable->comment)) {
									...
									<li><span>'. $entry->renderable->comment->count() .' '.get_string('comments', 'theme_pafco').'</span></li>';
                }
                $o .='
								</ul>
                <div class="ccn-blog-post-content-surround">
                  <p>'. format_text($entry->summary) .'</p>
							...
							<ul class="blog_post_share">
								<li><p>'.get_string('share', 'theme_pafco').'</p></li>
								<li><a href="'. $pafco_share_fb .'"><i class="fa fa-facebook"></i></a></li>
								<li><a href="'. $pafco_share_tw .'"><i class="fa fa-twitter"></i></a></li>
								<li><a href="'. $pafco_share_li .'"><i class="fa fa-linkedin"></i></a></li>
                <li><a href="'. $pafco_share_pi .'"><i class="fa fa-pinterest"></i></a></li>
								<li><a href="'. $pafco_share_vk .'"><i class="fa fa-vk"></i></a></li>
								<li><a href="'. $pafco_share_em .'"><i class="fa fa-envelope"></i></a></li>
							</ul>
						</div>';
            if($ccn_comments) {
              $o .='
              <div class="style2 mb30">
                <div class="block_comments">
                  <h4 class="aii_title">'.get_string('reviews', 'theme_pafco').'</h4>'. $ccn_comments->output(true) .'
                </div>
              </div>';
            }
					$o .='</div>';
