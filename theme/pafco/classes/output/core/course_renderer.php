...
namespace theme_pafco\output\core;
...
require_once($CFG->dirroot . '/theme/pafco/ccn/course_handler/ccn_course_handler.php');
...
class course_renderer extends \core_course_renderer {
  ...
  public function course_category($category) {
      ...
      if($ccnSubcategoryCount > 0) {
        if($ccnCourseCountRender === '') {
          ...
          $ccnCourseCountRender .= '<span class="ccn-text-divider"></span><span class="color-dark pr5">'.$ccnSubcategoryCount . '</span> ' . get_string('categories', 'theme_pafco'). ' ';
        }
       ...
       if (isset($PAGE->theme->settings->courseliststyle) && ($PAGE->theme->settings->courseliststyle == 2)) {
          $contenttext .= '
                      ...
                      <div class="tag">'.$ccnCourse->categoryName.'</div>
											<a class="tc_preview_course" href="'.$coursenamelink.'">'.get_string('preview_course', 'theme_pafco').'</a>
										</div>
									</div>
									<div class="details">
										<div class="tc_content">
  ...
  /**
   * ccnComm: BEGIN PAFCO OVERRIDES FOR FOCUS ACTIVITYNAV
   * Renders HTML to display a list of course modules in a course section
   * Also displays "move here" controls in Javascript-disabled mode
   *
   * This function calls {@link core_course_renderer::course_section_cm()}
  ...
  protected function frontpage_part($skipdivid, $contentsdivid, $header, $contents) {
    ...
    if (isset($PAGE->theme->settings->pafco_homepage_core) && ($PAGE->theme->settings->pafco_homepage_core === '1')) {
      return '';
    }
    if (strval($contents) === '') {
      return '';
  ...
  //         'query' => $value
  //     ];
  //     return $this->render_from_template('theme_pafco/ccn_mdl_310/ccn_course_search_form', $data);
  // }
