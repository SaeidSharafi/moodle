...
class tag_renderer extends \core_tag_renderer {
    ...
    public function tag_search_page($query = '', $tagcollid = 0) {
        ...
        if ($tagcloud->get_count()) {
            $searchresults = $this->output->render_from_template('theme_pafco/ccn_tag_index_cloud',
    ...
    public function tag_index_page($tag, $entities, $tagareaid, $exclusivemode, $fromctx, $ctx, $rec, $page) {
        ...
        $rv .= $this->output->render_from_template('theme_pafco/ccn_tag_taglist_large',
        ...
        foreach ($entities as $entity) {
            if (!empty($entity->hascontent)) {
              if(isset($_GET["ta"]) || isset($_GET["excl"])){
                $content .= $this->output->render_from_template('theme_pafco/ccn_tag_index_deeper', $entity->export_for_template($this->output));
              } else {
                $content .= $this->output->render_from_template('theme_pafco/ccn_tag_index', $entity->export_for_template($this->output));
              }
            }
        }
