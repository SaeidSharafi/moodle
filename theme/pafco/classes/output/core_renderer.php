    ...
    public function ccn_render_lang_menu() {
      ...
      foreach ($langs as $langtype => $langname) {
        $langArr[] = [
          ...
          'code' => $langtype,
          'icon' => $CFG->wwwroot.'/theme/pafco/pix/lang/'.strtoupper(str_replace("_", "-", $langtype)).'.svg',
        ];
      }
      ...
      return $this->render_from_template('theme_pafco/ccn_lang_menu', $context);
  }
...
return $output;
  ...
  public function user_menu($user = null, $withlinks = null) {
      ...
      $navitem->itemtype = 'link';
      $navitem->title = get_string('faq','theme_pafco');
      $navitem->titleidentifier = 'faq';
      $navitem->url = new moodle_url('/vums/faq.php');

      $navitemBlog = new stdClass();
      $navitemBlog->itemtype = 'link';
      $navitemBlog->title = get_string('blogs', 'blog');
      $navitemBlog->titleidentifier = 'blog';
      $navitemBlog->url = new moodle_url('/blog/index.php');

      $navitemBlog = new stdClass();
      $navitemBlog->itemtype = 'link';
      $navitemBlog->title = get_string('logout');
      $navitemBlog->titleidentifier = 'logout';
      $navitemBlog->url = new moodle_url('/login/logout.php',array('sesskey'=>sesskey()));

      $last = $opts->navitems[count($opts->navitems) -2];
      $opts->navitems = array_splice($opts->navitems,0,count($opts->navitems) -2);
      $opts->navitems[] = $navitem;
      $opts->navitems[] = $navitemBlog;
      $opts->navitems[] = $last;

      $opts->navitems = array_merge($primarynav,$opts->navitems);
      ...
      if ($withlinks) {
          ...
          foreach ($opts->navitems as $key => $value) {
            ...
            if(strpos($value->url, '/vums/faq.php')){
                $ccnMenuItemIcon = '<i class="icon fa fa-fw flaticon-creative-idea"></i>';
            }
            if(strpos($value->url, '/blog/index.php')){
            ...
            if(strpos($value->url, '/change_password.php')){
              $ccnMenuItemIcon = '<i class="icon fa fa-fw flaticon-key-1"></i>';
            }
            ...
            if(strpos($value->url, '/reportbuilder/index.php')){
              ...
              if(strpos($value->url, '/admin/search.php')){
                  $ccnMenuItemIcon = '<i class="icon fa fa-fw flaticon-settings"></i>';
              }
    ...
    public function user_menu_side($user = null, $withlinks = null) {
        global $USER, $CFG, $PAGE;
        require_once($CFG->dirroot . '/user/lib.php');
        ...
        $opts = user_get_user_navigation_info($user, $this->page);
        $primarynav=[];
        foreach ($PAGE->primarynav->children->getIterator() as $item){
            if ($item->key === 'home'){
                continue;
            }
            ...
            $navitem->titleidentifier = $item->key;
            $navitem->url = $item->action;

            array_push($primarynav,$navitem);
        }
        $navitem = new stdClass();
        $navitem->itemtype = 'link';
        $navitem->title = get_string('faq','theme_pafco');
        $navitem->titleidentifier = 'faq';
        $navitem->url = new moodle_url('/page/faq.php');
        ...
        if ($withlinks) {
            ...
            foreach ($opts->navitems as $key => $value) {
                ...
                if(strpos($value->url, '/page/faq.php')){
                    $ccnMenuItemIcon = '<i class="icon fa fa-fw flaticon-creative-idea"></i>';
                }
                if(strpos($value->url, '/blog/index.php')){
                    $ccnMenuItemIcon = '<i class="icon fa fa-fw flaticon-tablet"></i>';
  ...
  public function block(block_contents $bc, $region) {
      ...
      $ccnBlockInventory =  array(
        ...
        "pafco_course_details",
        "pafco_course_enrl_c",
        "pafco_course_feat_a",
        "pafco_course_features",
        "pafco_course_grid",
        "pafco_course_grid_2",
        "pafco_course_grid_3",
        "pafco_course_grid_4",
        ...
        "pafco_form",
        "pafco_gallery",
        "pafco_gallery_slider",
        "pafco_gallery_video",
        "pafco_globalsearch_n",
        "pafco_globalsearch_sb",
        "pafco_hero_1",
        "pafco_hero_2",
        "pafco_hero_3",
        "pafco_hero_4",
      ...
      // for Pafco blocks we want to default to programmatic styling instead of in-template
      ...
      if(in_array($context->type, $ccn_lc_vbCollection)) {
        $ccnActionUrl = $this->page->url->out(false, array('sesskey'=> sesskey(), 'bui_editid'=> $context->blockinstanceid, 'pafco_live_customizer'=> '1'));
      ...
      if($PAGE->pagelayout && ($PAGE->pagelayout == 'course' || $PAGE->pagelayout == 'incourse' || $PAGE->pagelayout == 'coursecategory')) {
        ...
        return $this->render_from_template('theme_pafco/ccn_block_dashboard_front', $context);
      } elseif($PAGE->pagelayout && ($PAGE->pagelayout == 'mydashboard' || $PAGE->pagelayout == 'admin')) {
        return $this->render_from_template('theme_pafco/ccn_block_dashboard_dash', $context);
      }
 ...
 protected function render_context_header(context_header $contextheader) {
  ...
  public function context_header($headerinfo = null, $headinglevel = 1) {
    ...
    public function render_login(\core_auth\output\login $form) {
        ...
        $logo = null;
        if (!empty(get_config('theme_pafco','headerlogo4'))) {
  ...
  public function full_header() {
    ...
    $ccnMdlVersion = (int)$ccnMdlVersion;
      ...
      if($ccnMdlVersion >= 400) {
        ...
        return $this->render_from_template('theme_pafco/ccn_mdl_400/full_header', $header);
      } else {
        if ($this->page->include_region_main_settings_in_header_actions() &&
                !$this->page->blocks->is_block_present('settings')) {
            // Only include the region main settings if the page has requested it and it doesn't already have
