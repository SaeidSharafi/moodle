...
namespace theme_pafco\output\core_user\myprofile;
...
class renderer extends \core_user\output\myprofile\renderer {
  ...
  public function render_tree(tree $tree) {
      ...
      if(!($ccn_page->id == $USER->id)) {
        if (!isset($SESSION->theme_pafco_counter)) {
          $SESSION->theme_pafco_counter = array();
        }
        if (!isset($SESSION->theme_pafco_counter[$ccn_page->id])) {
          $SESSION->theme_pafco_counter[$ccn_page->id] = array();
        }
        ...
        if (!isset($SESSION->theme_pafco_counter[$ccn_page->id]['time'])) {
          $sql = "SELECT MAX(time) AS mintime FROM {$CFG->prefix}theme_pafco_counter
              WHERE course = {$ccn_page->id}
              AND ip = '$ccn_ip'";

          $time = $DB->get_record_sql($sql);

          $SESSION->theme_pafco_counter[$ccn_page->id]['time'] = $time && $time->mintime ? $time->mintime : 0;
        }

        $ccn_increase = false;

        if ($SESSION->theme_pafco_counter[$ccn_page->id]['time'] < (time() - $ccn_time_difference)) {
          $dataobject = new \stdClass();
          $dataobject->ip = $ccn_ip;
          $dataobject->course = $ccn_page->id;
          $dataobject->time = time();
          $DB->insert_record('theme_pafco_counter', $dataobject, false);
          $SESSION->theme_pafco_counter[$ccn_page->id]['time'] = time();
          $ccn_increase = true;
        }
      ...
      if($PAGE->theme->settings->user_profile_layout != 1){ //Pafco Frontend
      ...
      $ccnProfileCountTable = 'theme_pafco_counter';
      ...
      <section class="our-team">
		    <div class="">';
          if($userAvatar && $PAGE->theme->settings->user_profile_layout != 1){ // PafcoFront ONLY
            $return .='
          ...
          <div class="row">
            <div class="'.$ccn_col_main.'">
              <div class="row">
                <div class="col-lg-12">';
                if(!in_array('description', $hiddenFields) && $userDescription && $PAGE->theme->settings->user_profile_layout != 1){ // PafcoFront
                $return .='
                ...
                } elseif(!in_array('description', $hiddenFields) && $PAGE->theme->settings->user_profile_layout == 1){ //Pafco Dash even without userDescription present
                  $return .='
                ...
                if(!in_array('mycourses', $hiddenFields) &&  $userLastCourses && $PAGE->theme->settings->user_profile_layout != 1){ //Pafco Frontend
                $return .='
                <div class="'.$ccn_col_main_block.'">
                  <div class="row">
                    <div class="col-lg-12">
                      <h3 class="r_course_title">'.$userFirst.get_string('apostrophe_s', 'theme_pafco').' '.get_string('last_accessed_courses', 'theme_pafco').'</h3>
                    </div>';
                    $ia = 0;
                        ...
                        $return .='
                          <div class="thumb">
                            ...
                            <div class="overlay">
                              ...
                              <a class="tc_preview_course" href="'.$courseLink.'">'.get_string('preview_course', 'theme_pafco').'</a>
                            </div>
                          </div>';
                        }
                        $return .='
              ...
              } elseif ($userLastCourses && $PAGE->theme->settings->user_profile_layout == 1){ //Pafco Dash
                $return .='
                <div class="'.$ccn_col_main_block.' p0">
                  <div class="'.$ccn_col_block_title.'">
                    <h4>'.$userFirst.get_string('apostrophe_s', 'theme_pafco').' '.get_string('last_accessed_courses', 'theme_pafco').'</h4>
                  </div>
                  <div class="container-fluid p-0">
                    <div class="my_course_content_list row no-gutters">';
                      ...
                      foreach ($userLastCourses as $course_id => $accessed) {
                        ...
                        if (!$contentimages){
                            $contentimages = '<img class="img-whp" alt="'.$courseTitle.'" src="/theme/pafco/pix/course-default.jpg"/>';;
                        }
                        $return .='
                            ...
                            <div class="overlay">
                              <ul class="mb0">
                                <li class="list-inline-item">
                                  <a class="mcc_view" href="'.$courseLink.'">'.get_string('view', 'theme_pafco').'</a>
                                </li>
                              </ul>
                            </div>
                          </div>';
...
<div class="small">
    <span class="sr-only">'.get_string('aria:courseprogress', 'block_myoverview').'</span>'.
              ...
              if($userSkype || $userIcq || $userYahoo | $userAim || $userMsn){
                $return .='
  							<p>'.get_string('socialmedia', 'theme_pafco').'</p>
            ...
            $return .= '
              ...
              ';
                if($teachingStudentCount){
                  $return .='
    							<p>'.get_string('studentsiamteaching', 'theme_pafco').'</p>
    							<i>'.$teachingStudentCount.'</i>';
                }
                if($teachingCoursesCount){
                  $return .='
    							<p>'.get_string('coursesiamteaching', 'theme_pafco').'</p>
    							<i>'.$teachingCoursesCount.'</i>';
                }
                  $return .='<p>'.get_string('coursesiamtaking', 'theme_pafco').'</p>
    							<i>'.$enrolmentCount.'</i>';
                if($ccnProfileCount){
                  $return .='<p>'.get_string('profileviews', 'theme_pafco').'</p>
    							<i>'.$ccnProfileCount.'</i>';
                }
                $return .='
						</div>
  ...
  public function render_category(category $category) {
      ...
      foreach ($nodes as $node) {
          ...
          if ($category->name === 'contact' && $node->name !== 'editprofile') {
              continue;
          }
          if(!($node->content)) {
      ...
      if ($category->name === 'contact') {
          $return .= '<div class="content style-link"><div class="circle"></div><h4 class="edu_stats edu_stats_link"><a href="/login/change_password.php">'
              .get_string('changepassword', 'moodle').'</a></h4></div>';
      }
      $return .='</div></div></div>';
      $return .= '</div></div></div>';
      return $return;
