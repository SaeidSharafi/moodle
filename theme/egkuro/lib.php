 ...
 * @return null
 */
function theme_egkuro_page_init(moodle_page $page) {
    $page->requires->jquery();
    $page->requires->js('/theme/egkuro/javascript/theme.js');
}
 ...
 * @return string $css
 */
function theme_egkuro_process_css($css, $theme) {
    ...
    $css = theme_egkuro_set_fontwww($css);
    $css = theme_egkuro_get_pattern_color($css, $theme);
    $css = theme_egkuro_set_customcss($css, $customcss);
 ...
 * @return bool
 */
function theme_egkuro_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    static $theme;

    if (empty($theme)) {
        $theme = theme_config::load('egkuro');
    }
 ...
 * @return string
 */
function theme_egkuro_serve_css($filename) {
    global $CFG;
    if (!empty($CFG->themedir)) {
        $thestylepath = $CFG->themedir . '/egkuro/style/';
    } else {
        $thestylepath = $CFG->dirroot . '/theme/egkuro/style/';
    }
    ...
    if ((($ifmodifiedsince) && (strtotime($ifmodifiedsince) == $lastmodified)) || $etagheader == $etagfile) {
        theme_egkuro_send_unmodified($lastmodified, $etagfile);
    }
    theme_egkuro_send_cached_css($thestylepath, $filename, $lastmodified, $etagfile);
}
 ...
 *
 */
function theme_egkuro_send_unmodified($lastmodified, $etag) {
 ...
 * @param string $etag
 */
function theme_egkuro_send_cached_css($path, $filename, $lastmodified, $etag) {
    global $CFG;
 ...
 */
function theme_egkuro_set_customcss($css, $customcss) {
 ...
 */
function theme_egkuro_get_html_for_settings(renderer_base $output, moodle_page $page) {
    global $CFG;
    $return = new stdClass;
 ...
 */
/*function theme_egkuro_set_fontwww() {
    global $CFG, $PAGE;

    $themewww = $CFG->wwwroot."/theme";
    $theme = theme_config::load('egkuro');
    $fontwww = '$fontwww: "'. $themewww.'/egkuro/fonts/"'.";\n";
    return $fontwww;
}*/
function theme_egkuro_set_fontwww($css) {
    global $CFG, $PAGE;
    ...
    $tag = '[[setting:fontwww]]';
    $theme = theme_config::load('egkuro');
    $css = str_replace($tag, $themewww . '/egkuro/fonts/', $css);
    return $css;
}
 ...
 * @return image $logo
 */
function theme_egkuro_get_logo_url($type = 'header') {
    global $OUTPUT;
    static $theme;
    if (empty($theme)) {
        $theme = theme_config::load('egkuro');
 ...
 * @return image $logo
 */
function theme_egkuro_get_banner_url() {
    global $OUTPUT;
    static $theme;
    if (empty($theme)) {
        $theme = theme_config::load('egkuro');
    ...
    $logo = $theme->setting_file_url('loginbanner', 'banner');
    $logo = empty($logo) ? $OUTPUT->image_url('home/logo', 'theme') : $logo;
    return $logo;
}
 ...
 * @return null
 */
function theme_egkuro_render_slideimg($p, $sliname) {
    global $PAGE, $OUTPUT;

    $nos = theme_egkuro_get_setting('numberofslides');
    $i = $p % 3;
    $slideimage = $OUTPUT->image_url('home/slide' . $i, 'theme');

    // Get slide image or fallback to default.
    if (theme_egkuro_get_setting($sliname)) {
 ...
 * @return bool
 */
function theme_egkuro_get_setting($setting, $format = true) {
    global $CFG;
 ...
 * @return string
 */
function theme_egkuro_theme_url() {
    global $CFG, $PAGE;
 ...
 */
function theme_egkuro_generate_links($menuname = '') {
    global $CFG, $PAGE;
    $htmlstr = '';
    $menustr = theme_egkuro_get_setting($menuname);
 ...
 * @return array
 */
function theme_egkuro_hidden_courses_ids() {
    global $DB;
    $hcourseids = array();
 ...
 * @return string
 */
function theme_egkuro_strip_html_tags($text) {
    $text = preg_replace(
            array(
 ...
 * @return string $out
 */
function theme_egkuro_course_trim_char($str, $n = 500, $endchar = '&#8230;') {
    if (strlen($str) < $n) {
        return $str;
    }
 ...
 * @return string
 */
function theme_egkuro_lang($key = '') {
    $pos = strpos($key, 'lang:');
    if ($pos !== false) {
        list($l, $k) = explode(":", $key);
        if (get_string_manager()->string_exists($k, 'theme_egkuro')) {
            $v = get_string($k, 'theme_egkuro');
            return $v;
        } else {
            return $key;
        }
...
function theme_egkuro_get_pattern_color($css, $type = '') {
    ...
    $patterncolors = array(
            'default' => array(
                    ...
                    'collapsed_empty' => $CFG->wwwroot . '/theme/egkuro/pix/default/t/collapsed_empty',
                    'collapsed' => $CFG->wwwroot . '/theme/egkuro/pix/default/t/collapsed',
                    'collapsed_rtl' => $CFG->wwwroot . '/theme/egkuro/pix/default/t/collapsed_rtl',
                    'expanded' => $CFG->wwwroot . '/theme/egkuro/pix/default/t/expanded'
            ),
            ...
            '1' => array(
                    ...
                    'collapsed_empty' => $CFG->wwwroot . '/theme/egkuro/pix/cs01/t/collapsed_empty.png',
                    'collapsed' => $CFG->wwwroot . '/theme/egkuro/pix/cs01/t/collapsed.png',
                    'collapsed_rtl' => $CFG->wwwroot . '/theme/egkuro/pix/cs01/t/collapsed_rtl.png',
                    'expanded' => $CFG->wwwroot . '/theme/egkuro/pix/cs01/t/expanded.png'
            ),
            ...
            '2' => array(
                    ...
                    'collapsed_empty' => $CFG->wwwroot . '/theme/egkuro/pix/cs02/t/collapsed_empty.png',
                    'collapsed' => $CFG->wwwroot . '/theme/egkuro/pix/cs02/t/collapsed.png',
                    'collapsed_rtl' => $CFG->wwwroot . '/theme/egkuro/pix/cs02/t/collapsed_rtl.png',
                    'expanded' => $CFG->wwwroot . '/theme/egkuro/pix/cs02/t/expanded.png'
            ),
            ...
            '3' => array(
                    ...
                    'collapsed_empty' => $CFG->wwwroot . '/theme/egkuro/pix/cs03/t/collapsed_empty.png',
                    'collapsed' => $CFG->wwwroot . '/theme/egkuro/pix/cs03/t/collapsed.png',
                    'collapsed_rtl' => $CFG->wwwroot . '/theme/egkuro/pix/cs03/t/collapsed_rtl.png',
                    'expanded' => $CFG->wwwroot . '/theme/egkuro/pix/cs03/t/expanded.png'
            ),
            ...
            '4' => array(
                    ...
                    'collapsed_empty' => $CFG->wwwroot . '/theme/egkuro/pix/cs04/t/collapsed_empty.png',
                    'collapsed' => $CFG->wwwroot . '/theme/egkuro/pix/cs04/t/collapsed.png',
                    'collapsed_rtl' => $CFG->wwwroot . '/theme/egkuro/pix/cs04/t/collapsed_rtl.png',
                    'expanded' => $CFG->wwwroot . '/theme/egkuro/pix/cs04/t/expanded.png'
            ),

            '5' => array(
                    'color_primary' => '#673AB7',
                    'color_secondary' => '#7341ce',
                    'color_blackcurrant_approx' => '#382738',
                    'color_plum_approx' => '#764076',
                    'color_blackcurrant_90_approx' => 'rgba(56, 39, 56, 0.9)',
                    'color_french_lilac_approx' => '#ead1ea',
    ...
    $selectedpattern = theme_egkuro_get_setting('patternselect');
 ...
 * @return type|string
 */
function theme_egkuro_get_hexa($hexa, $opacity) {
    if (!empty($hexa)) {
...
function theme_egkuro_get_main_scss_content($theme) {
    global $CFG, $USER;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_egkuro', 'preset', 0, '/', $filename))) {
    ...
    $pre = file_get_contents($CFG->dirroot . '/theme/egkuro/scss/pre.scss');
    // Post CSS - this is loaded AFTER the main scss but before the extra scss from the setting.
    $post = file_get_contents($CFG->dirroot . '/theme/egkuro/scss/post.scss');
    $nonadmin = "\n" . file_get_contents($CFG->dirroot . '/theme/egkuro/scss/nonadmin.scss');
 ...
 */
function theme_egkuro_extend_flat_navigation(\flat_navigation $flatnav) {
    //theme_egkuro_add_coursesections_to_navigation($flatnav);

    theme_egkuro_delete_menuitems($flatnav);

    theme_egkuro_add_menuitem($flatnav);
    theme_egkuro_change_frontpage($flatnav);
}

function theme_egkuro_change_frontpage(\flat_navigation $flatnav) {

    try {
        if (!isloggedin() || isguestuser()) {
            return;
        }
        foreach ($flatnav as $key => &$item) {
            //trigger_error(print_r($item,true), E_NOTICE); // Script stopped after this line...

            if ($item->key == 'home') {
                //trigger_error(print_r($item,true), E_NOTICE); // Script stopped after this line...
                $item->text = get_string("mycourses", 'theme_egkuro');
                $item->icon = new pix_icon('t/viewdetails', '');
            }
        }
 ...
 *
 */
function theme_egkuro_add_menuitem(\flat_navigation $flatnav) {
    global $COURSE;

    try {
        //        ///******** Add Items *********////
        //        $options = theme_egkuro_get_setting('addnav', 'format_html');
        ...
        //                switch ($temp[0]) {
        //                    case 'REG':
        //                        $registered[$key]['title']= $temp[1];
        //                        $registered[$key]['key']= $temp[2];
        //                        $registered[$key]['url']= $temp[3];
        //                        if (array_key_exists(4,$temp)){
        //                            $guest[$key]['icon'] =   new pix_icon('i/'.$temp[4], '');
        //                        }else{
        //                            $guest[$key]['icon'] =   new pix_icon('i/home', '');
        //                        }
        //                        break;
        //                    case 'GUEST':
        //                        $guest[$key]['title']= $temp[1];
        //                        $guest[$key]['key']= $temp[2];
        //                        $guest[$key]['url']= $temp[3];
        //                        if (array_key_exists(4,$temp)){
        //                            $guest[$key]['icon'] =   new pix_icon('i/'.$temp[4], '');
        //                        }else{
        //                            $guest[$key]['icon'] =   new pix_icon('i/home', '');
        ...
        //        $itemstoadd = [];
        //        if (isloggedin() && !isguestuser()) {
        //            $itemstoadd = $registered;
        //        }else if (isguestuser()){
        //            $itemstoadd = $guest;
        //        }
        //        foreach ($itemstoadd as $item){
        //            $url = new \moodle_url($item['url']);
        //            $string_title = explode(":", $item['title']);
        //            $title="Item";
        //            if (count($string_title) > 1){
        //                if ($string_title[0] == 'lang'){
        //                    $title = get_string($string_title[1], 'theme_egkuro');
        //                }else{
        //                    $title=$string_title[1];
        //                }
        //            }
        //            $node = [
        //                'action' => $url,
        ...
        //                'key' => $item['key']
        //            ];
        //            //$node = navigation_node::create($title, $url);
 ...
 * @param flat_navigation $flatnav
 */
function theme_egkuro_delete_menuitems(\flat_navigation $flatnav) {
    global $USER, $COURSE;
    $options = theme_egkuro_get_setting('customizenav', 'format_html');
    $options = explode(PHP_EOL, $options);
    //    var_dump($options);
    $registered = [];
    $guest = [];
 ...
 * @param flat_navigation $flatnav
 */
function theme_egkuro_add_coursesections_to_navigation(\flat_navigation $flatnav) {
    global $PAGE;
    ...
    if ($PAGE->course->format != 'singleactivity') {
        $coursesectionsoptions = [
                'text' => get_string('coursesections', 'theme_egkuro'),
                'shorttext' => get_string('coursesections', 'theme_egkuro'),
    ...
    if ($mycourses) {
        ...
        $flatnav->add($mycourses, 'calendar');
    }
}
///**
...
// * @return bool
// */
//function theme_egkuro_has_certificates_plugin() {
