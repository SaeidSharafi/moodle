...
tput cup 10 57
echo -e "\e[1;35m 4. Update Moodle to 4.1 latest (3.10+)\e[m"
...
#}
  ...
  if [ "$install_db" = "y" ]; then
    ...
    echo ""
    sudo -u nginx /usr/bin/php /var/www/site/html/admin/cli/install_database.php --lang=en --agree-license --fullname="$fullname" --shortname="$shortname" --adminuser="$adminuser" --adminpass="$adminpassword" --adminemail="$adminemail"
  else
    sudo -u nginx /usr/bin/php /var/www/site/html/admin/cli/upgrade.php --lang=en --non-interactive
  fi
...
updateM4() {
  ...
  sudo -u nginx /usr/bin/php admin/cli/upgrade.php --lang=en --non-interactive
  rm -rf pafco-v4.1-latest.zip
  echo -e "in case using 4m, \e[1;31mDO NOT FORGET\e[m to replace \e[1;33m<ADDRESS>\e[m and \e[1;33m<TOKEN>\e[m in login\index.php and admin\soap.php"
}
...
while true; do
  ...
  *)
    tput cup 17 58
    tput el
    echo -e "\e[0;31mInvalid input\e[m" >&2
    ;;
  esac
done
