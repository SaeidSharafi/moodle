...
class enrol_acecr_testcase extends advanced_testcase {
    ...
    public function test_basics() {
        $this->assertFalse(enrol_is_enabled('acecr'));
        $plugin = enrol_get_plugin('acecr');
        $this->assertInstanceOf('enrol_acecr_plugin', $plugin);
        $this->assertEquals(ENROL_EXT_REMOVED_SUSPENDNOROLES, get_config('enrol_acecr', 'expiredaction'));
    }
    ...
    public function test_sync_nothing() {
        ...
        $acecrplugin->sync(new null_progress_trace());
    }
    ...
    public function test_expired() {
        ...
        /** @var enrol_acecr_plugin $acecrplugin  */
        $acecrplugin = enrol_get_plugin('acecr');
        ...
        $acecrplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $acecrplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $acecrplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);

        $acecrplugin->enrol_user($instance2, $user1->id, $studentrole->id, 0, 0);
        $acecrplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now-60*60);
        $acecrplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now+60*60);

        $acecrplugin->enrol_user($instance3, $user1->id, $teacherrole->id, $now-60*60*24*7, $now-60);
        $acecrplugin->enrol_user($instance3, $user4->id, $teacherrole->id);
        ...
        $acecrplugin->set_config('expiredaction', ENROL_EXT_REMOVED_KEEP);
        $code = $acecrplugin->sync($trace);
        $this->assertSame(0, $code);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        $this->assertEquals(9, $DB->count_records('role_assignments'));


        $acecrplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $acecrplugin->sync($trace);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        ...
        $acecrplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
