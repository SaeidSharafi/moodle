...
use enrol_acecr\privacy\provider;
use core_privacy\local\request\writer;

/**
 * Class enrol_acecr_privacy_provider_testcase.
 *
 * @copyright  2018 Shamim Rezaie <shamim@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_acecr_privacy_provider_testcase extends \core_privacy\tests\provider_testcase {

    /** @var stdClass A user whose email address matches the business field in some of the AcecrPayment transactions. */
    ...
    /** @var stdClass A user whose email address matches the receiver_email field in some of the AcecrPayment transactions. */
    ...
    protected function setUp(): void {
        ...
        $acecrplugin->enrol_user($enrolinstance1, $this->student1->id, $studentrole->id);
        $this->create_enrol_acecr_record(
            $this->businessuser1,
            $this->receiveruser1,
            $this->course1,
            $this->student1,
        ...
        $acecrplugin->enrol_user($enrolinstance2, $this->student2->id, $studentrole->id);
        ...
        // First in course1.
        $acecrplugin->enrol_user($enrolinstance1, $this->student12->id, $studentrole->id);
        $this->create_enrol_acecr_record(
            $this->businessuser2,
            $this->receiveruser1,
            $this->course1,
        ...
        // Then in course2.
        $acecrplugin->enrol_user($enrolinstance2, $this->student12->id, $studentrole->id);
        $this->create_enrol_acecr_record(
            $this->businessuser2,
            $this->receiveruser2,
            $this->course2,
        ...
        $acecrplugin->enrol_user($enrolinstance1, $this->student3->id, $studentrole->id);
        $this->create_enrol_acecr_record(
            $this->businessuser3,
            $this->receiveruser3,
            $this->course3,
    ...
    public function test_get_metadata() {
        $collection = new collection('enrol_acecr');
        $newcollection = provider::get_metadata($collection);
        $itemcollection = $newcollection->get_collection();
        $this->assertCount(2, $itemcollection);

        $location = reset($itemcollection);
        $this->assertEquals('mellat.com', $location->get_name());
        $this->assertEquals('privacy:metadata:enrol_acecr:mellat_com', $location->get_summary());
        ...
        $this->assertEquals('enrol_acecr', $table->get_name());
        $this->assertEquals('privacy:metadata:enrol_acecr:enrol_acecr', $table->get_summary());
    ...
    public function test_export_user_data() {
        ...
        $this->export_context_data_for_user($this->student1->id, $coursecontext1, 'enrol_acecr');
        $writer = writer::with_context($coursecontext1);
        $this->assertTrue($writer->has_any_data());

        $data = $writer->get_data([get_string('transactions', 'enrol_acecr')]);
    ...
    public function test_export_user_data_not_enrolled() {
        ...
        // Export all of the data for the context.
        $this->export_context_data_for_user($this->student2->id, $coursecontext1, 'enrol_acecr');
        $writer = writer::with_context($coursecontext1);
    ...
    public function test_export_user_data_no_enrolment() {
        ...
        // Export all of the data for the context.
        $this->export_context_data_for_user($this->student0->id, $coursecontext1, 'enrol_acecr');
        $writer = writer::with_context($coursecontext1);
        $this->assertFalse($writer->has_any_data());
    }

    public function test_export_user_data_multiple_acecr_history() {
        $coursecontext2 = context_course::instance($this->course2->id);

        $this->setUser($this->student2);
        // Export all of the data for the context.
        $this->export_context_data_for_user($this->student2->id, $coursecontext2, 'enrol_acecr');
    ...
    public function test_delete_data_for_all_users_in_context() {
        ...
        // Before deletion, we should have 2 AcecrPayment transactions in course1 and 3 AcecrPayment transactions in course2.
        $this->assertEquals(
                2,
                $DB->count_records('enrol_acecr', ['courseid' => $this->course1->id])
        );
        $this->assertEquals(
                3,
                $DB->count_records('enrol_acecr', ['courseid' => $this->course2->id])
        );

        // Delete data based on context.
        provider::delete_data_for_all_users_in_context($coursecontext1);

        // After deletion, AcecrPayment transactions in course1 should have been deleted.
        $this->assertEquals(
                0,
                $DB->count_records('enrol_acecr', ['courseid' => $this->course1->id])
        );
    ...
    public function test_delete_data_for_all_users_in_context_multiple_transactions() {
        ...
        // After deletion, AcecrPayment transactions in course2 should have been deleted.
        $this->assertEquals(
                2,
                $DB->count_records('enrol_acecr', ['courseid' => $this->course1->id])
        );
    ...
    public function test_delete_data_for_user_from_single_context() {
        ...
        // Before deletion, we should have 2 AcecrPayment transactions (1 of them for student12) in course1
        // and 3 AcecrPayment transactions (1 of them for student12) in course2.
        $this->assertEquals(
                2,
                $DB->count_records('enrol_acecr', ['courseid' => $this->course1->id])
        );
        $this->assertEquals(
                1,
                $DB->count_records('enrol_acecr', ['courseid' => $this->course1->id, 'userid' => $this->student12->id])
        );
        $this->assertEquals(
                3,
                $DB->count_records('enrol_acecr', ['courseid' => $this->course2->id])
        ...
        $this->assertEquals(
                1,
                $DB->count_records('enrol_acecr', ['courseid' => $this->course2->id, 'userid' => $this->student12->id])
        );

        // Delete data for user.
        $contextlist = new \core_privacy\local\request\approved_contextlist($this->student12, 'enrol_acecr',
                [$coursecontext1->id]);
        provider::delete_data_for_user($contextlist);

        // After deletion, AcecrPayment transactions for student12 in course1 should have been deleted.
        $this->assertEquals(
                1,
                $DB->count_records('enrol_acecr', ['courseid' => $this->course1->id])
        );
    ...
    public function test_delete_data_for_user_from_multiple_context() {
        ...
        // After deletion, AcecrPayment enrolment data for student12 in both course1 and course2 should have been deleted.
        $this->assertEquals(
                1,
                $DB->count_records('enrol_acecr', ['courseid' => $this->course1->id])
        );
    ...
    public function test_delete_data_for_user_for_business_user() {
        ...
        // Before deletion, we should have 5 AcecrPayment enrolments.
        // 3 of which paid to businessuser1 and 2 of which paid to businessuser2.
        $this->assertEquals(
                3,
                $DB->count_records('enrol_acecr', ['business' => $this->businessuser1->email])
        );
        $this->assertEquals(
                2,
                $DB->count_records('enrol_acecr', ['business' => $this->businessuser2->email])
        );

        // Delete data for user in $coursecontext1.
        $contextlist = new \core_privacy\local\request\approved_contextlist($this->businessuser1, 'enrol_acecr',
                [$coursecontext1->id]);
        provider::delete_data_for_user($contextlist);

        // After deletion, AcecrPayment enrolment data for businessuser1 in course1 should have been deleted.
        $this->assertEquals(
                0,
                $DB->count_records('enrol_acecr', ['courseid' => $this->course1->id, 'business' => $this->businessuser1->email])
        );
        $this->assertEquals(
                2,
                $DB->count_records('enrol_acecr', ['business' => $this->businessuser1->email])
        ...
        $this->assertEquals(
                1,
                $DB->count_records('enrol_acecr', ['courseid' => $this->course1->id, 'business' => ''])
        );
        $this->assertEquals(
                2,
                $DB->count_records('enrol_acecr', ['business' => $this->businessuser2->email])
    ...
    public function test_delete_data_for_user_for_receiver_user() {
        ...
        $this->assertEquals(
                2,
                $DB->count_records('enrol_acecr', ['receiver_email' => $this->receiveruser1->email])
        );
        $this->assertEquals(
                3,
                $DB->count_records('enrol_acecr', ['receiver_email' => $this->receiveruser2->email])
        );

        // Delete data for user.
        $contextlist = new \core_privacy\local\request\approved_contextlist($this->receiveruser1, 'enrol_acecr',
                [$coursecontext1->id]);
        provider::delete_data_for_user($contextlist);

        // After deletion, AcecrPayment enrolment data for receiveruser1 in course1 should have been deleted.
        $this->assertEquals(
                0,
                $DB->count_records('enrol_acecr', ['receiver_email' => $this->receiveruser1->email])
        );
        $this->assertEquals(
                2,
                $DB->count_records('enrol_acecr', ['receiver_email' => ''])
        );
        $this->assertEquals(
                3,
                $DB->count_records('enrol_acecr', ['receiver_email' => $this->receiveruser2->email])
    ...
    /**
     * Helper function to create an enrol_acecr record.
     *
     ...
     * @param   String      $txnid The Acecr txnid to use
     * @param   int         $time The txn time
     */
    protected function create_enrol_acecr_record($business, $receiver, $course, $user, $enrol, $txnid, $time) {
        global $DB;
        ...
        $DB->insert_record('enrol_acecr', $acecrdata);
    }
    ...
    public function test_get_users_in_context() {
        ...
        $userlist1 = new \core_privacy\local\request\userlist($coursecontext1, 'enrol_acecr');
        ...
        $userlist2 = new \core_privacy\local\request\userlist($coursecontext2, 'enrol_acecr');
        ...
        $userlist3 = new \core_privacy\local\request\userlist($coursecontext3, 'enrol_acecr');
    ...
    public function test_delete_data_for_users() {
        ...
        $this->assertEqualsCanonicalizing(
                ...
                $DB->get_fieldset_select('enrol_acecr', 'courseid', 'userid = ?', [$this->student12->id])
        );
        $this->assertEqualsCanonicalizing(
                [$this->course1->id, $this->course2->id, $this->course2->id],
                $DB->get_fieldset_select('enrol_acecr', 'courseid', 'business = ?',
        ...
        $approveduserlist = new \core_privacy\local\request\approved_userlist($coursecontext1, 'enrol_acecr',
                [$this->student12->id, $this->businessuser1->id]);
        provider::delete_data_for_users($approveduserlist);

        // After deletion, AcecrPayment transactions for student12 in course1 should have been deleted.
        // Now, we should have 1 AcecrPayment transaction (which is not related to student12) in course1.
        // There should still be 3 AcecrPayment transactions (1 of them for student12) in course2.
    ...
    public function test_delete_data_for_users_business() {
        ...
        $this->assertEquals(3, $DB->count_records('enrol_acecr', ['business' => $this->businessuser1->email]));
        $transactions = $DB->get_records('enrol_acecr', [
        ...
        $this->assertEquals(2, $DB->count_records('enrol_acecr', ['business' => $this->businessuser1->email]));
        $this->assertEquals(0, $DB->count_records('enrol_acecr', [
            'courseid' => $this->course1->id,
            'business' => $this->businessuser1->email
        ]));
        ...
        $newtransaction = $DB->get_record('enrol_acecr', ['id' => $transaction->id]);
    ...
    public function test_delete_data_for_users_receiver() {
        ...
        $this->assertEquals(2, $DB->count_records('enrol_acecr', ['receiver_email' => $this->receiveruser1->email]));
        $transactions = $DB->get_records('enrol_acecr', [
            'courseid' => $this->course1->id,
        ...
        $this->assertEquals(0, $DB->count_records('enrol_acecr', ['receiver_email' => $this->receiveruser1->email]));
