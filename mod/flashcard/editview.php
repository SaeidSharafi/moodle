...
if ($data = $mform->get_data()) {
    ...
    if ($data->what == 'update') {
        ...
        if ($DB->get_dbfamily() === 'mssql') {
            ...
            $deckusers = $DB->get_records_sql(
                "SELECT DISTINCT userid FROM {flashcard_card} WHERE flashcardid = ? ORDER BY userid",
                [$flashcard->id]
            );
        } else {
            // Query for other DBMS (e.g., MySQL)
            $deckusers = $DB->get_records('flashcard_card', ['flashcardid' => $flashcard->id], 'userid', 'DISTINCT userid, userid');
        }
