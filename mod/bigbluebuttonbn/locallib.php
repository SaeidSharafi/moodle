...
function bigbluebuttonbn_get_recording_data_row_types($recording, $bbbsession) {
    ...
    $href = $recording['meta_download'];
    $name = $recording['meta_bbb-recording-name'];
    if ($href){
        $linkattributes = array(
            'id' => 'recording-play-download' . $recording['recordID'],
            'class' => 'btn btn-sm btn-default mt-1',
            'download' => $name,
            'target' => '_blank'
        );
        $recordingtypes .= $OUTPUT->action_link($href, get_string('download','bigbluebuttonbn')
            , null, $linkattributes);
    }
