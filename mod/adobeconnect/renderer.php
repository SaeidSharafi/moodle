...
class mod_adobeconnect_renderer extends plugin_renderer_base {
    ...
    function display_controllers($adobeconnect,$cmid, $scoid, $groupmode, $usrprincipal, $show_attendance, $show_recordings) {
        ...
        $data = (object) [
                'last_sync_record' => $last_rec,
                'last_sync_attendance' => $last_att,
                'cmid' => $cmid,
                'scoid' => $scoid,
                'groupmode' => $groupmode,
    ...
    function display_meeting_recording($recordings, $cmid, $groupid, $sourcescoid, $showoffline) {
        ...
        foreach ($recordings as $recording) {
            ...
            $url_offline = null;
            if ($showoffline) {
                $url_offline = 'joinrecording.php?mode=offline&id=' . $cmid . '&recording=' . $recording->recordingscoid .
                        '&groupid=' . $groupid . '&sesskey=' . $USER->sesskey;
            }
            $param = array('target' => '_blank');
    ...
    function display_meeting_attendees($attendees, $cmid, $groupid, $sourcescoid) {
        ...
        foreach ($attendees as $key => $attendee) {
            if (!empty($attendee)) {
                ...
                $name = html_entity_decode($attendee->email);
                $session_name = html_entity_decode($attendee->session_name);
                $participant_name = html_entity_decode($attendee->participant_name);
                $user_fields = get_user_custom_fields($attendee->email);
                ...
                foreach ($attendee->attendances as $index => $attendance) {
                    $start_hour[] = $this->convertDate($attendance->start_date, '%H:%M', false);
                    //                    $end_hour[] = $this->convertDate($attendance->end_date, '%H:%M', false);
                    $start_dates[] = $this->convertDate($attendance->start_date);
                    //                    $end_dates[] = $this->convertDate($attendance['end_date);
                    $join_date[] = $this->convertDate($attendance->start_date, 'strftimedaydate');
                    $duration = $this->diffDate($attendance->start_date, $attendance->end_date, 'h:i:s', false);

                    $e->add($duration);
                    if (!$this->compareDate($attendance->start_date, $attendance->end_date)) {
                        $hour = false;
                        $end_dates[] = $this->convertDate($attendance->end_date);
                    } else {
                        $end_dates[] = $this->convertDate($attendance->end_date, '%H:%M', false);
                    }

                }
                $diff_houres = ($f->diff($e)->days * 24 + (int) $f->diff($e)->format('%H'));
                if ($diff_houres > 24) {
                    // Will enter here now :)
                    $duration = $diff_houres . $f->diff($e)->format(":%I:%S");
                } else {
                ...
                $row = [
                        ...
                        'use_hour' => $hour,
                        'join_dates' => $join_date,
                        'start_hours' => $start_hour,
                        'end_hours' => $end_hour,
                        'start_dates' => $start_dates,
                        'end_dates' => $end_dates,
                        'duration' => $duration,
                        'count_log' => count($join_date)
        ...
        //if (count($rows) > 0){
        //    $fields = array();
        //    foreach ($rows[0]['user_fields'] as $field){
        //        $fields[] = $field->title;
        //    }
        //}
        $data = (object) [
    ...
    function display_meeting_attendees_csv($attendees, $cmid, $groupid, $sourcescoid) {
        global $CFG, $USER;
        ...
        foreach ($attendees as $key => $attendee) {
            if (!empty($attendee)) {
                ...
                $session_name = html_entity_decode($attendee->session_name);
                $participant_name = html_entity_decode($attendee->participant_name);
                $user_fields = get_custom_fields($attendee->email);
                $join_date = array();
                $start_hour = array();
                $end_hour = array();
                $start_dates = array();
        ...
        return $this->render_from_template('adobeconnect/attendance_csv', $data);

        //return $html;
        //$html .= html_writer::link($url, get_string('removemychoice','choice'));
    }

    function display_meeting_attendees_csv_perrow($attendees, $cmid, $groupid, $sourcescoid) {
        global $CFG, $USER;
