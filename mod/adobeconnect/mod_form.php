...
class mod_adobeconnect_mod_form extends moodleform_mod
{
    ...
    function definition()
    {
        ...
        $mform->addElement('text', 'name',
            get_string('adobeconnectname', 'adobeconnect'),
            array('size' => '64'));
        ...
        $this->standard_intro_elements(get_string('adobeconnectintro',
            'adobeconnect'));
        ...
        $mform->addElement('header', 'adobeconnectfieldset',
            get_string('adobeconnectfieldset', 'adobeconnect'));

        // Meeting URL
        $attributes = array('size' => '20');
        $mform->addElement('text', 'meeturl',
            get_string('meeturl', 'adobeconnect'), $attributes);
        ...
        $meetingpublic = array(
            1 => get_string('public', 'adobeconnect'),
            0 => get_string('private', 'adobeconnect'),
            2 => get_string('protected', 'adobeconnect')
            );
        $mform->addElement('select', 'meetingpublic',
            get_string('meetingtype', 'adobeconnect'), $meetingpublic);
        $mform->setDefault('meetingpublic', 2);
        ...
        ksort($templates);
        $mform->addElement('select', 'templatescoid',
            get_string('meettemplates', 'adobeconnect'), $templates);
        ...
        $mform->addElement('date_time_selector', 'starttime',
            get_string('starttime', 'adobeconnect'));
        $mform->addElement('date_time_selector', 'endtime',
            get_string('endtime', 'adobeconnect'));
        $mform->setDefault('endtime', strtotime('+2 hours'));

        $mform->addElement('selectyesno', 'showoffline',
            get_string('showoffline', 'adobeconnect'));
        $mform->setDefault('showoffline', $configs->showoffline);
    ...
    function validation($data, $files)
    {
        ...
        $namematches = aconnect_meeting_exists($aconnect, $meetfldscoid,
            $filter);
        ...
        if (!empty($usrfldscoid)) {
            $namematches = $namematches + aconnect_meeting_exists($aconnect,
                    $usrfldscoid, $filter);
        }
        ...
        if ((0 != substr_count($url, '/')) and (false !== strpos($url, '/',
                    1))
        ) {
            $errors['meeturl'] = get_string('invalidadobemeeturl',
                'adobeconnect');
        }

        $filter = array('filter-like-url-path' => $url);
        $urlmatches = aconnect_meeting_exists($aconnect, $meetfldscoid,
            $filter);

        /// Search the user's adobe connect folder
        if (!empty($usrfldscoid)) {
            $urlmatches = $urlmatches + aconnect_meeting_exists($aconnect,
                    $usrfldscoid, $filter);
        }
        ...
        if ($data['groupmode'] > 0) {
            ...
            if (empty($crsgroups)) {
                $errors['groupmode'] = get_string('missingexpectedgroups',
                    'adobeconnect');
            }
        }
        ...
        if (empty($data['update'])) {
            ...
            if ($DB->record_exists('adobeconnect', $params)) {
                $errors['name'] = get_string('duplicatemeetingname',
                    'adobeconnect');
                return $errors;
            }
            ...
            foreach ($namematches as $matchkey => $match) {
                if (0 == substr_compare($match->name, $data['name'].'_', 0,
                        strlen($data['name'].'_'), false)
                ) {
                    $errors['name'] = get_string('duplicatemeetingname',
                        'adobeconnect');
                }
            ...
            foreach ($urlmatches as $matchkey => $match) {
                ...
                if (0 == substr_compare($matchurl, $url.'_', 0,
                        strlen($url.'_'), false)
                ) {
                    $errors['meeturl'] = get_string('duplicateurl',
                        'adobeconnect');
                }
            }
            ...
            $sql
                = "SELECT meetingscoid, groupid FROM {adobeconnect_meeting_groups} "
                .
            ...
            foreach ($urlmatches as $matchkey => $match) {
                if (!array_key_exists($match->scoid, $grpmeetings)) {
                    if (0 == substr_compare($match->url, $url.'_', 0,
                            strlen($url.'_'), false)
                    ) {
                        $errors['meeturl'] = get_string('duplicateurl',
                            'adobeconnect');
