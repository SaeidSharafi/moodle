...
function adobeconnect_add_instance($adobeconnect)
{
    ...
    if (0 != $adobeconnect->groupmode) { // Allow for multiple groups
        ...
        foreach ($crsgroups as $crsgroup) {
            ...
            } else if ($adobeconnect->meetingpublic == 1) {
                aconnect_update_meeting_perm($aconnect, $meetingscoid, ADOBE_MEETPERM_PUBLIC);
            }else if ($adobeconnect->meetingpublic == 2){
                aconnect_update_meeting_perm($aconnect, $meetingscoid, ADOBE_MEETPERM_PROTECTED);
            }
...
function adobeconnect_update_instance($adobeconnect)
{
    ...
    if ($timechange or $urlchange or $namechange) {
        ...
        foreach ($grpmeetings as $scoid => $grpmeeting) {
            ...
            if (empty($adobeconnect->meetingpublic)) {
                aconnect_update_meeting_perm($aconnect, $meetingobj->scoid, ADOBE_MEETPERM_PRIVATE);
            } else if ($adobeconnect->meetingpublic == 1) {
                aconnect_update_meeting_perm($aconnect, $meetingobj->scoid, ADOBE_MEETPERM_PUBLIC);
            }else if ($adobeconnect->meetingpublic == 2){
                aconnect_update_meeting_perm($aconnect, $meetingobj->scoid, ADOBE_MEETPERM_PROTECTED);
            }
