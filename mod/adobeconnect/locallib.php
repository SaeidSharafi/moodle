...
function syncRecordings($meetscoids, $cmid, $groupmode, $usrprincipal, $isAuto) {
    ...
    if (!has_capability('mod/adobeconnect:viewrecordings', $context, $USER->id)) {
        return array('status' => 0, 'msg' => "you don't have permission for this action", 'data' => '');
    }
    ...
    if (!$isAuto) {
        ...
        if ((time() - $ins->last_sync_record) < 3600) {
            return array('status' => 0, 'msg' => "no need to synch", 'data' => '');
    ...
    if (!empty($meetscoids)) {
        ...
        return array('status' => 1, 'msg' => "Ops successful", 'data' => json_encode($data));
    }

    return array('status' => 0, 'msg' => "meetscoids is empty", 'data' => '');
...
function syncAttendances($meetscoids, $instanceid, $isAuto) {
    ...
    if (!has_capability('mod/adobeconnect:viewattendees', $context, $USER->id)) {
        ...
        if($isAuto){
            return array('status' => 0, 'msg' => $msg, 'data' => '');
        }
        return array('status' => -1, 'msg' => $msg, 'data' => '');
    }
    if ($isAuto) {
    ...
    if (!empty($meetscoids)) {
        ...
        if (count($rows) == 0) {
            ...
            return array('status' => -2, 'msg' => "Ops successful, no attendance found", 'data' => json_encode($data));
        }
        foreach ($rows as $row) {
            ...
            foreach ($row->dates as $index => $attendance) {
                ...
                $attended->join_time = userdate($attended->start_date, "%H:%M");;
                $attended->exit_time = userdate($attended->end_date, "%H:%M");
                $attendee->duration += ($attended->end_date - $attended->start_date);
                ...
                $attendee->duration = secondsTooTime($attendee->duration);
        ...
        return array('status' => 0, 'msg' => "Ops failed, no meeting sco found", 'data' => '');
    }
}
    ...
    if ($recording_scoid) {
        ...
        if ($res['status']) {
            try {
                ...
                return array(
                        'status' => 0,
                        'msg' => 'recording removed from Adobeconnect but removing form database failed ',
                        'data' => ''
                );
                $transaction->rollback($e);
            }
...
function getAttendances($context, $instanceid) {
    ...
    try {
        ...
        return toNestedObject($attendees);
